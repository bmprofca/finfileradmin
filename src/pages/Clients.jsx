...
const ClientAvatar = ({ client, size = 'md' }) => {
  ...
  if (client.image) {
    return <img src={client.image} alt={client.full_name} className={`${cls} rounded-lg object-cover shrink-0`} />;
  }
  return (
    ...
    <ManagementHub
      ...
      actions={
        <Button onClick={handleCreateNew} variant="primary" className="flex items-center gap-2 text-sm py-1.5 bg-blue-600 hover:bg-blue-700">
          <Plus size={16} /><span className="hidden md:block">Add Client</span>
        </Button>
      }
    >
