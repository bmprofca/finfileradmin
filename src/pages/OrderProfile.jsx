...
const InfoItem = ({ icon: Icon, label, value }) => (
  <div className="flex items-start gap-2 rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50 p-4">
    ...
    <div className="min-w-0 flex-1">
      <div className="text-[10px] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 leading-none mb-1">
        {label}
      </div>
      <div className="text-sm font-medium text-gray-800 dark:text-gray-100 leading-snug break-words">
        {value ?? "N/A"}
      </div>
    </div>
  </div>
...
const FILE_TYPE_MAP = {
  jpg: {
    ...
    icon: Image,
    color:
      "text-blue-500 bg-blue-50 dark:bg-blue-900/30 border-blue-200 dark:border-blue-800",
  },
  jpeg: {
    label: "Image",
    icon: Image,
  ...
  csv: {
    ...
    color:
      "text-green-600 bg-green-50 dark:bg-green-900/30 border-green-200 dark:border-green-800",
  },
  xlsx: {
    label: "Spreadsheet",
    icon: FileSpreadsheet,
  ...
  zip: {
    ...
    icon: File,
    color:
      "text-gray-600 bg-gray-100 dark:bg-gray-700/50 border-gray-300 dark:border-gray-600",
  },
  rar: {
    label: "Archive",
    icon: File,
...
const getFileTypeInfo = (doc) =>
  FILE_TYPE_MAP[getDocumentExtension(doc)] || DEFAULT_FILE_TYPE;
...
const StaffManagementModal = ({
  ...
  const isInitial = () =>
    JSON.stringify([...rightStaff.map((s) => s.username)].sort()) ===
    JSON.stringify([...initialRightStaff].sort());
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col gap-4 lg:flex-row md:flex-row items-stretch">
          <div className="flex-1 border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-gray-50 dark:bg-gray-900/30">
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                <UserPlus size={14} className="text-indigo-500" /> Available (
                {leftStaff.length})
              </h4>
              {leftStaff.length > 0 && (
                <button
          ...
          <div className="flex-1 border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-gray-50 dark:bg-gray-900/30">
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                <UserCheck size={14} className="text-green-500" /> Assigned (
                {rightStaff.length})
              </h4>
              {rightStaff.length > 0 && (
                <button
...
const CaModal = ({
  ...
  const [selected, setSelected] = useState(
    order?.ca?.username ? { value: order.ca.username, label: `${order.ca.name || order.ca.username} · ${order.ca.mobile || ""}` } : null
  );

  const caOptions = allCAs.map((c) => ({
    value: c.username,
    label: `${c.name || c.full_name || c.username} · ${c.mobile || ""}`,
  }));

  const handleSubmit = () =>
    onSubmit({ order_id: order.order_id, ca_username: selected?.value || "" });

  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title={`${order?.ca ? "Manage CA" : "Assign CA"} · ${order?.order_name || ""}`}
      icon={Briefcase}
      size="md"
      contentClassName="p-5"
      closeText="Cancel"
      footer={
        ...
        >
          <Briefcase size={14} /> {isSubmitting ? "Saving..." : order?.ca ? "Update CA" : "Assign CA"}
        </button>
      }
    >
      <div className="space-y-4">
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Select a CA to assign to this order. Clear the selection to remove the current CA.
        </p>
        <SelectField
          options={caOptions}
          value={selected}
          onChange={setSelected}
          placeholder={caLoading ? "Loading CAs..." : "Select a CA..."}
          isLoading={caLoading}
          isClearable
        />
      </div>
...
const CaFeesModal = ({ order, onClose, onSubmit, isSubmitting }) => {
  const [fees, setFees] = useState(order?.ca_fees ?? "");
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSubmit({ order_id: order.order_id, ca_fees: Number(fees) || 0 });
  };
  ...
  return (
    <Modal
      ...
      footer={
        ...
        >
          <IndianRupee size={14} /> {isSubmitting ? "Saving..." : "Update Fees"}
        </button>
      }
    >
      <form id="ca-fees-form-profile" onSubmit={handleSubmit} className="space-y-4">
        <div>
        ...
        {order?.ca && (
          <div className="flex items-center gap-2 p-3 rounded-lg bg-violet-50 dark:bg-violet-900/20 border border-violet-200 dark:border-violet-800 text-sm text-violet-700 dark:text-violet-300">
            <Briefcase size={14} /> CA: <span className="font-semibold">{order.ca.name || order.ca.username}</span>
          </div>
        )}
      </form>
    </Modal>
  ...
  const formContent = (
    ...
    >
      ...
      <div className="flex items-center justify-end gap-3 pt-2">
        ...
        >
          <Edit size={14} /> {isSubmitting ? "Updating..." : "Update Order"}
        </button>
      </div>
    </form>
  );
  ...
  return (
    <Modal
      ...
      footer={
        <button
          type="submit"
          form="order-update-form-profile"
          disabled={isSubmitting}
          className="px-5 py-2.5 rounded-lg bg-indigo-600 dark:bg-indigo-500 text-white text-sm font-semibold hover:bg-indigo-700 dark:hover:bg-indigo-600 transition-all flex items-center gap-2 disabled:opacity-50"
        >
          <Edit size={14} /> {isSubmitting ? "Updating..." : "Update Order"}
  ...
  return (
    <Modal
      ...
      footer={
        <button
          type="submit"
          form="order-status-form-profile"
          disabled={isSubmitting}
      ...
      >
        <div>
          ...
          <SelectField
            ...
            value={
              orderStatusOptions.find((o) => o.value === form.status) ||
...
export default function OrderProfile() {
  ...
  const activeTab = TAB_IDS.includes(tabParam) ? tabParam : "basic";
  const setActiveTab = (tabId) => {
    navigate(tabId === "basic" ? `/orders/${orderId}` : `/orders/${orderId}/${tabId}`);
  };
  ...
  // Documents (received from client, type = "in")
  ...
  // Uploaded documents (sent to client, type = "out")
  const [outDocuments, setOutDocuments] = useState([]);
  const [outDocsLoading, setOutDocsLoading] = useState(false);
  const [outDocsFetched, setOutDocsFetched] = useState(false);
  const [outDocsPage, setOutDocsPage] = useState(1);
  const [outDocsTotal, setOutDocsTotal] = useState(0);
  const [outDocsLimit, setOutDocsLimit] = useState(20);
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    try {
      const res = await apiCall(
        `/api/admin/documents/list?order_id=${orderId}&type=in&page_no=${docsPage}&limit=${docsLimit}`,
        "GET",
      );
      const data = await res.json();
      if (data.success) {
        ...
        setDocsTotal(
          data.pagination?.total_records || data.pagination?.total || 0,
        );
      }
    } catch {
  ...
  const fetchOutDocuments = useCallback(async () => {
    setOutDocsLoading(true);
    try {
      const res = await apiCall(
        `/api/admin/documents/list?order_id=${orderId}&type=out&page_no=${outDocsPage}&limit=${outDocsLimit}`,
        "GET",
      );
      const data = await res.json();
      if (data.success) {
        setOutDocuments(data.data.documents || []);
        setOutDocsTotal(
          data.pagination?.total_records || data.pagination?.total || 0,
        );
      }
      ...
      setOutDocsLoading(false);
      setOutDocsFetched(true);
    }
  }, [orderId, outDocsPage, outDocsLimit]);
  ...
  useEffect(() => {
    ...
    if (
      lastDocsFetchRef.current.page === docsPage &&
      lastDocsFetchRef.current.limit === docsLimit &&
      docsFetched
    )
      return;
  ...
  const lastOutDocsFetchRef = useRef({ page: null, limit: null });
  useEffect(() => {
    if (activeTab !== "upload") return;
    if (
      lastOutDocsFetchRef.current.page === outDocsPage &&
      lastOutDocsFetchRef.current.limit === outDocsLimit &&
      outDocsFetched
    )
      return;
    lastOutDocsFetchRef.current = { page: outDocsPage, limit: outDocsLimit };
    fetchOutDocuments();
  }, [activeTab, outDocsPage, outDocsLimit, fetchOutDocuments, outDocsFetched]);
  ...
  const handleRefresh = async () => {
    ...
    if (activeTab === "upload") {
      setOutDocsFetched(false);
      await fetchOutDocuments();
    }
    setRefreshing(false);
  };
  ...
  const ensureCasFetched = async () => {
    if (!casFetchedRef.current) {
      setCaLoading(true);
      try {
        const res = await apiCall("/api/admin/cas/list", "GET");
        const data = await res.json();
        if (data.success) {
          setAllCAs(data.data.cas || data.data.staffs || []);
  ...
  const handleAssignCa = async (payload) => {
    setSaving(true);
    try {
      const res = await apiCall("/api/admin/orders/assign/ca", "PUT", payload);
      const data = await res.json();
      if (data.success) {
        toast.success(payload.ca_username ? "CA assigned successfully" : "CA removed successfully");
        setCaModalOpen(false);
        fetchOrder();
      } else toast.error(data.message || "Failed to assign CA");
    } catch {
      toast.error("An error occurred.");
    } finally {
      setSaving(false);
  ...
  const handleUpdateCaFees = async (payload) => {
    setSaving(true);
    try {
      const res = await apiCall("/api/admin/orders/ca-fees", "PUT", payload);
      const data = await res.json();
      if (data.success) {
        toast.success("CA fees updated successfully");
        setCaFeesModalOpen(false);
        fetchOrder();
      } else toast.error(data.message || "Failed to update CA fees");
    } catch {
      toast.error("An error occurred.");
    } finally {
      setSaving(false);
  ...
  const handleDownloadStatement = async () => {
    ...
    try {
      ...
      const res = await apiCall(
        `/api/admin/orders/download-payments/${orderId}`,
        "GET",
      );
      const data = await res.json();
  ...
  const handleDownloadDocument = async (doc) => {
    const fileName =
      doc.file_name || doc.document_name || doc.name || "document";
    const toastId = toast.loading("Downloading...");
    try {
      const res = await apiCall(
        `/api/admin/documents/download/${doc.document_id}?source=order`,
        "GET",
      );
  ...
  /* ─── Upload Documents (Upload tab) ─── */
  const processUploadFiles = async (selectedFilesList) => {
  ...
  const handleUploadDrop = (e) => {
    ...
    e.stopPropagation();
    setIsDragging(false);
    if (!isUploadingFiles) processUploadFiles(e.dataTransfer.files);
  };
  ...
  const updateUploadFileName = (index, newName) => {
    setUploadFiles((current) =>
      current.map((f, i) => (i === index ? { ...f, name: newName } : f)),
    );
  };

  const uploadTotalSize = uploadFiles.reduce(
  ...
  const handleSubmitUpload = async (e) => {
    ...
    const missingName = uploadFiles.some((f) => !f.name.trim());
    if (missingName) {
      toast.error("Enter a document name for all files.");
      return;
    }

    const payload = uploadFiles.map((f) => ({ name: f.name.trim(), url: f.url }));

    setUploadSubmitting(true);
    try {
      const res = await apiCall(
        `/api/admin/orders/upload-document/${orderId}`,
        "POST",
        payload,
      );
      const data = await res.json();
  ...
  const closeUploadModal = () => {
    if (uploadSubmitting || isUploadingFiles) return;
    setUploadModalOpen(false);
    setUploadFiles([]);
  };

  /* ─── Document columns ─── */
  const makeDocColumns = (page, limit) => [
    {
      key: "serial_no",
      label: "#",
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return (
          <span
            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold border ${info.color}`}
          >
            {info.label}
  ...
  const docColumns = makeDocColumns(docsPage, docsLimit);
  const outDocColumns = makeDocColumns(outDocsPage, outDocsLimit);
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return (
          ...
          >
            {(row.status || "unknown").replace(/\b\w/g, (l) => l.toUpperCase())}
          </span>
        );
      },
    },
  ...
  return (
    <div>
      {/* ─── Sticky Header + Tabs ─── */}
      <div className="relative top-0 z-10 -mt-2 md:-mt-6 bg-gray-50/95 dark:bg-gray-900/95 backdrop-blur-sm">
        {/* Header */}
        {order && (
        ...
        {!orderLoading && order && (
          <AnimatePresence mode="wait">
              ...
              >
                <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-6">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-lg border border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-900/20 px-3 py-1.5 text-xs font-semibold text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <InfoItem
                      icon={User}
                      label="Client"
                      value={order.client_name || order.client_username}
                    />
                    <InfoItem
                      icon={Calendar}
                      label="Created"
                    ...
                    <InfoItem
                      icon={Calendar}
                      label="Modified"
                      value={formatDateTime(order.modify_date)}
                    />
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    />
                    {order.ca && order.ca_fees !== undefined && (
                      <InfoItem
                        icon={IndianRupee}
                        label="CA Fees"
                        value={formatCurrency(order.ca_fees)}
                      />
                    )}
                    <InfoItem
                      icon={Tag}
                      label="Discount Type"
                      value={(order.discount_type || "N/A").replace(
                        /\b\w/g,
                        (l) => l.toUpperCase(),
                      )}
                    />
                    <InfoItem
                      ...
                      value={
                        order.discount_type === "percentage"
                          ? `${order.discount_percentage}% (${formatCurrency(order.discount_value)})`
                ...
                {order.firm ? (
                  <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      <InfoItem
                        icon={Building2}
                        label="Firm Name"
                        value={order.firm.name}
                      />
                      <InfoItem
                        icon={Tag}
                        label="Firm Type"
                        value={(order.firm.type || "N/A").replace(
                          /\b\w/g,
                          (l) => l.toUpperCase(),
                        )}
                      />
                      <InfoItem
                        icon={Hash}
                        label="PAN No."
                        value={order.firm.pan_no}
                      />
                      <InfoItem
                        icon={Hash}
                        label="GST No."
                        value={order.firm.gst_no}
                      />
                      {order.firm.vat_no && (
                        <InfoItem
                          icon={Hash}
                          label="VAT No."
                          value={order.firm.vat_no}
                        />
                      )}
                      {order.firm.tan_no && (
                        <InfoItem
                  ...
                  <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-6">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      No firm/business associated with this order.
                    </p>
                  </div>
                )}
                ...
                {order.remark && (
                  <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-6">
                    <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-3 flex items-center gap-2">
                      <FileText className="text-indigo-500" size={20} /> Remark
                    </h3>
              ...
              >
                ...
                <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-6">
                  <div className="flex flex-col sm:flex-row items-start gap-6">
                    {order.client_image ? (
                      <img
                        src={order.client_image}
                        alt={order.client_name}
                        className="w-20 h-20 rounded-xl object-cover border-2 border-gray-200 dark:border-gray-700 shrink-0"
                      />
                    ) : (
                      <div className="w-20 h-20 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white text-3xl font-bold shrink-0">
                        {(order.client_first_name || order.client_name || "?")
                          .charAt(0)
                          .toUpperCase()}
                      </div>
                    )}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        {order.client_status !== undefined && (
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${order.client_status === 1 ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300" : "bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300"}`}
                          >
                    ...
                    <button
                      onClick={() =>
                        navigate(`/clients/${order.client_username}`)
                      }
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-900/20 text-sm font-semibold text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors shrink-0"
                    >
                ...
                <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-6">
                  <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-6 flex items-center gap-2">
                    <User className="text-indigo-500" size={20} /> Personal
                    Details
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <InfoItem
                      icon={User}
                      label="First Name"
                      value={order.client_first_name}
                    />
                    <InfoItem
                      icon={User}
                      label="Middle Name"
                      value={order.client_middle_name}
                    />
                    <InfoItem
                      icon={User}
                      label="Last Name"
                      value={order.client_last_name}
                    />
                    <InfoItem
                      icon={Mail}
                      label="Email"
                    ...
                    <InfoItem
                      icon={Phone}
                      label="Mobile"
                      value={order.client_mobile}
                    />
                    <InfoItem
                      icon={Hash}
                      label="Username"
                      value={order.client_username}
                    />
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-6">
                  <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4 flex items-center gap-2">
                    <Phone className="text-indigo-500" size={20} /> Quick
                    Contact
                  </h3>
                  <div className="flex flex-wrap gap-3">
                    {order.client_mobile && (
                      <a
                        href={`tel:${order.client_mobile}`}
                        className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-sm font-semibold text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors"
                      >
                    ...
                    {order.client_mobile && (
                      <a
                        href={`https://wa.me/${order.client_mobile.replace(/\D/g, "")}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-sm font-semibold text-green-700 dark:text-green-300 hover:bg-green-100 dark:hover:bg-green-900/40 transition-colors"
                      >
                    ...
                    {order.client_email && (
                      <a
                        href={`mailto:${order.client_email}`}
                        className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-sm font-semibold text-blue-700 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors"
                      >
            ...
            {activeTab === "documents" && (
              ...
              >
                {docsLoading ? (
                  <PageContentSkeleton rows={5} columns={3} />
                ) : documents.length === 0 ? (
            ...
            {activeTab === "staffs" && (
              ...
              >
                {/* CA Card */}
                {order.ca && (
                  <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-6 mb-4">
                    <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2 mb-6">
                      <Briefcase className="text-indigo-500" size={20} /> Assigned CA
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      <div className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
                        {order.ca.image ? (
                          <img
                            src={order.ca.image}
                            alt={order.ca.name || order.ca.username}
                          ...
                          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-white text-sm font-bold shrink-0">
                            {(order.ca.name || order.ca.username || "?").charAt(0).toUpperCase()}
                          </div>
                        )}
                        <div className="min-w-0 flex-1">
                          <p className="font-medium text-sm text-gray-800 dark:text-gray-200 truncate">
                            {order.ca.name || order.ca.username}
                          </p>
                          <div className="text-xs text-gray-500 dark:text-gray-400 mt-1 space-y-1">
                            {order.ca.mobile && (
                              <p className="flex items-center gap-1"><Phone size={11} /> {order.ca.mobile}</p>
                            )}
                            {order.ca.email && (
                              <p className="flex items-center gap-1"><Mail size={11} /> {order.ca.email}</p>
                            )}
                          </div>
                        </div>
                      </div>
                ...
                <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-6">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
                      <Users className="text-indigo-500" size={20} /> Assigned
                      Staff ({order.assigned_staff?.length || 0})
                    </h3>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={openCaModal}
                        className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${order.ca ? "bg-violet-600 dark:bg-violet-500 text-white hover:bg-violet-700 dark:hover:bg-violet-600" : "bg-orange-500 dark:bg-orange-500 text-white hover:bg-orange-600 dark:hover:bg-orange-600"}`}
                      >
                        <Briefcase size={14} /> {order.ca ? "Manage CA" : "Assign CA"}
                      </button>
                      {order.ca && (
                        <button
                          onClick={openCaFeesModal}
                          className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-teal-600 dark:bg-teal-500 text-white text-sm font-semibold hover:bg-teal-700 dark:hover:bg-teal-600 transition-colors"
                        >
                          <IndianRupee size={14} /> CA Fees
                        </button>
                      )}
                      <button
                        onClick={openStaffModal}
                        className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 dark:bg-indigo-500 text-white text-sm font-semibold hover:bg-indigo-700 dark:hover:bg-indigo-600 transition-colors"
                      >
                  ...
                  {order.assigned_staff && order.assigned_staff.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {order.assigned_staff.map((s) => (
                        <div
                          key={s.username}
                          className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50"
                        >
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
                        Click "Manage Staff" to assign staff to this order.
                      </p>
                    </div>
                  )}
                </div>
            ...
            {activeTab === "payments" && (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div
                    className={`bg-white dark:bg-gray-900 rounded-xl shadow-sm border p-4 text-center ${order.due_amount > 0 ? "border-amber-100 dark:border-amber-900/30" : "border-emerald-100 dark:border-emerald-900/30"}`}
                  >
                    <p
                      className={`text-xs font-semibold uppercase tracking-wider mb-1 ${order.due_amount > 0 ? "text-amber-600 dark:text-amber-400" : "text-emerald-600 dark:text-emerald-400"}`}
                    >
                      Due Amount
                    </p>
                    <p
                      className={`text-xl font-bold ${order.due_amount > 0 ? "text-amber-700 dark:text-amber-300" : "text-emerald-700 dark:text-emerald-300"}`}
                    >
                      {formatCurrency(order.due_amount)}
                    </p>
                  </div>
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-teal-200 dark:border-teal-800 bg-teal-50 dark:bg-teal-900/20 text-sm font-semibold text-teal-700 dark:text-teal-300 hover:bg-teal-100 dark:hover:bg-teal-900/40 transition-colors"
                  >
            ...
            {activeTab === "status" && (
              ...
              >
                <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-6">
                  <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-6 flex items-center gap-2">
                    <RefreshCw className="text-indigo-500" size={20} /> Order
                    Status
                  </h3>
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-emerald-600 dark:bg-emerald-500 text-white text-sm font-semibold hover:bg-emerald-700 dark:hover:bg-emerald-600 transition-colors"
                  >
                  ...
                  <>
                    <div className="flex items-center justify-between">
                      <h3 className="text-sm font-semibold text-gray-600 dark:text-gray-300">
                        Documents shared with client ({outDocsTotal})
                      </h3>
                      <button
                        onClick={() => setUploadModalOpen(true)}
                        className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 dark:bg-indigo-500 text-white text-sm font-semibold hover:bg-indigo-700 dark:hover:bg-indigo-600 transition-colors"
                    ...
                    {outDocsTotal > 0 && (
                      <PaginationComponent
                        currentPage={outDocsPage}
                        totalItems={outDocsTotal}
                        itemsPerPage={outDocsLimit}
                        onPageChange={setOutDocsPage}
                        onLimitChange={(l) => {
                          setOutDocsLimit(l);
                          setOutDocsPage(1);
                        }}
        ...
        footer={
          <>
            <button
              ...
              className="rounded-lg border border-gray-300 bg-white px-5 py-2.5 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
            >
              Clear
            </button>
            <button
              type="submit"
              form="order-upload-documents-form"
              disabled={
                uploadSubmitting ||
                isUploadingFiles ||
            ...
            >
              <Upload size={16} />
              {uploadSubmitting ? "Submitting..." : "Submit Documents"}
            </button>
          </>
        }
      >
        <form
          id="order-upload-documents-form"
        ...
        >
          <div>
            ...
            >
              {isUploadingFiles ? (
                <Loader2
                  className="mb-2 text-indigo-500 dark:text-indigo-400 animate-spin"
                  size={30}
                />
              ) : (
                <Upload
                  className={`mb-2 transition-transform ${isDragging ? "scale-110 text-indigo-600 dark:text-indigo-300" : "text-indigo-500 dark:text-indigo-400"}`}
                  size={30}
                />
              )}
              <span className="text-sm font-semibold text-gray-800 dark:text-gray-100">
              ...
              <span className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                Images, PDF, CSV, or other documents
              </span>
              <input
                type="file"
                multiple
              ...
              <div className="space-y-3">
                {uploadFiles.map((item, index) => (
                  <div
                    ...
                    className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-800"
                  >
                    <div className="flex min-w-0 flex-1 items-center gap-3 w-full">
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        <input
                          ...
                          value={item.name}
                          onChange={(e) =>
                            updateUploadFileName(index, e.target.value)
                          }
